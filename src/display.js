import { saveNewProject, updateProject, getProjectByID, getToDoItemByID, updateToDoItem } from "./storage";
...
function deleteToDoItem(projectID, toDoID) {
    ...
    if (project) {
        const updatedToDoList = project.toDoList.filter(item => item.internalID !== toDoID);
